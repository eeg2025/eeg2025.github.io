...
Challenge 2 Contestants will develop models to predict the externalizing factors from EEG recordings. The evaluation metric for this challenge is also the normalized root mean square error:

$$
\mathcal{S}_{2}=\operatorname{nRMSE}\left(\mathbf{Y}_{2, \text { true }}, \mathbf{Y}_{2, \text { pred }}\right)
$$

With $\mathbf{Y}_{2, \text { true }}$ being the labels vector containing the externalizing factors obtained from questionaries, $\mathbf{Y}_{2, \text { pred }}$ containing the externalizing factors predicted from the EEG recordings, and $\mathcal{S}_{2}$ the score for challenge 2.
